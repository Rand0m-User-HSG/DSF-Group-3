...
for (i in 1:fold) {
  ...
  y_classified = apply(cbind(rep(1,round(nrow(X_matrix)/fold)), x_i) %*% beta_one_vs_all , 1, FUN=which.max)
  empirical_error_for_fold_i = length(which(y_classified != y_i)) / (round(nrow(X_matrix)/fold))
