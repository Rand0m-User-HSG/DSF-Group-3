...
#################################################################### we random sample datapoint to solve the unbalance
...
for (i in 1:fold) {
  ...
  y_classified[(1+(i-1)*nrow(X_matrix)/fold):(i*nrow(X_matrix)/fold)] <- pred
  cv_error_boosting[i] = mean(abs(Y_test - as.integer(pred)))
}
...
for (i in 1:fold){
  ...
  X_k <- X_sample[(1 + (i-1)*nrow(X_sample)/fold):(i*(nrow(X_sample)/fold)),]
  Y_k <- to_categorical(Y_sample[(1 + (i-1)*nrow(X_sample)/fold):(i*(nrow(X_sample)/fold))])
  X_test <- X_matrix[-((1 + (i-1)*nrow(X_matrix)/fold) : (i*(nrow(X_matrix)/fold))),]
  Y_test <- Y_vector[-((1 + (i-1)*nrow(X_matrix)/fold) : (i*(nrow(X_matrix)/fold)))]
